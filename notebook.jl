...
$$g(s, \theta) = (\mathcal{R}_{\mu} f)(s, \theta) = \int_{-\infty}^{\infty} f(s\,\pmb{\omega} + t\,\pmb{\omega}^{\perp}) \mathrm{d}t$$
...
angles = range(0, 2π, angle_slider + 1)[begin:angle_slider]

# ╔═╡ 5821296e-082a-4ab0-9141-fe6e9b0632cc
@time sinogram_j = radon(julia_logo, angles);

# ╔═╡ 3956c102-8dd7-4901-a4e2-0b7988640194
begin
	p = plot(sinogram_j[:, 1], label="0°", title="Projections for different angles", ylabel="intensity", xlabel="pixel position")
	try
		plot!(sinogram_j[:, 2], label="$(round(rad2deg(angles[2]), digits=0))°")
	catch
		1
	end
	p
...
[RadonKA.jl](https://github.com/roflmaostc/RadonKA.jl) implemented a simple yet sufficiently fast Radon transform

* Runs on CPU, CUDA, ... with [KernelAbstractions.jl](https://github.com/JuliaGPU/KernelAbstractions.jl)
*  For 2D and 3D arrays 
* parallel `radon` and `backproject` (`?RadonParallelCircle`)
* attenuated `radon` and `backproject` (see the parameter `μ`)
...
* Different geometries such as without and with index matching bath
* Ray and wave optics
...
SwissVAMyKnife = "fafdf525-54c7-48ec-ae29-528bd4cccfab"
...
git-tree-sha1 = "fc2133d2b337e62d8bc9e18277c34b5d7aabd673"
...
[[deps.SwissVAMyKnife]]
deps = ["CUDA", "ChainRulesCore", "DiffImageRotation", "FFTW", "FileIO", "FourierTools", "HDF5", "ImageIO", "ImageShow", "IndexFunArrays", "JLD2", "NDTools", "NNlib", "Optim", "Parameters", "Pkg", "Plots", "RadonKA", "Statistics", "Tar", "WaveOpticsPropagation", "Zygote"]
git-tree-sha1 = "f2382d411c4efde6379489fef7d8e620d6397d43"
